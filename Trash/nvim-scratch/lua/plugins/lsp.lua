    ...
    },
    {
        ...
        config = function()
            ...
            local on_attach = function(client, bufnr)
                ...
                vim.keymap.set('n', 'gd', '<cmd>lua vim.lsp.buf.declaration()<CR>', with_desc("Go to declaration", bufnr)) -- Go to declaration
                vim.keymap.set('n', 'gD', '<cmd>lua vim.lsp.buf.definition()<CR>', with_desc("Go to definition", bufnr))   -- Go to definition
                vim.keymap.set('n', 'gr', '<cmd>lua vim.lsp.buf.references()<CR>', with_desc("Go to references", bufnr))   -- Go to references
                vim.keymap.set('n', 'gI', '<cmd>lua vim.lsp.buf.implementation()<CR>',
                    with_desc("Go to implementation", bufnr))                                                              -- Go to implementation
                ...
                vim.keymap.set('n', '[d', '<cmd>lua vim.diagnostic.goto_prev()<CR>',
                    with_desc("Go to previous diagnostic", bufnr)) -- Go to previous diagnostic
                vim.keymap.set('n', ']d', '<cmd>lua vim.diagnostic.goto_next()<CR>',
                    with_desc("Go to next diagnostic", bufnr))     -- Go to next diagnostic
                vim.keymap.set('n', '<leader>e', '<cmd>lua vim.diagnostic.open_float()<CR>',
                    with_desc("Show line diagnostics", bufnr))     -- Show line diagnostics
                vim.keymap.set('n', '<leader>cf', '<cmd>lua vim.lsp.buf.format({ async = true })<CR>',
                    with_desc("Format buffer", bufnr))             -- Format buffer
            end
